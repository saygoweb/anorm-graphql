...
**Tech Stack:** PHP `^7.4 || ^8.0`, `saygoweb/anorm ^3.2`, `webonyx/graphql-php ^14.8`, `simpod/graphql-utils ^0.5.3`, `php-di/php-di ^6.0`, `wp-cli/php-cli-tools`, PHPUnit 9.6, phpcs (PSR-12), phpstan level 5, Docker (PHP 7.4 CLI + MariaDB 10.11).
...
Every source and test file in this plan was written and **run before the plan was written**: 65 PHPUnit tests passing on PHP 7.4 against MariaDB 10.11 (the no-DB suites also on PHP 8.3), phpcs clean, phpstan level 5 clean. The docker port script was dry-run (it produces a script that passes `bash -n` and prints its help), but the stack itself was **not** brought up from it, and `composer.json`, `StackTest` and the composer scripts were not executed. That is why Task 1 is the one task whose code you should expect to have to debug. The code blocks are therefore to be **transcribed exactly**, not improved. If a step fails, the likeliest causes, in order, are: a transcription slip, a dependency resolving to a different version than the one verified, or the environment. Diagnose in that order before touching the code's logic.
...
| inthefish is on webonyx 0.13 / anorm 1.5 | it is outside this package's supported range; emdc-events and saygoweb.com-my are inside it |
...
| Task 2 Runtime without a database | **haiku** | Three small files moved near-verbatim, with unit tests. |
...
Expected: the image builds (a few minutes the first time), `composer install` runs inside the container and creates `vendor/`, and `info` prints PHP 7.4.x and the database port. If `composer install` cannot resolve, read the conflict: the constraints in `composer.json` are the ones the two supported APIs already resolve with, so a conflict means a typo.
...
**Files:**
- Create: `src/GraphQLUtils.php`, `src/Mapper.php`, `src/Type/MangoInput.php`
- Create: `test/Fixtures/Type/ResolvingType.php`
- Test: `test/runtime/MapperTest.php`, `test/runtime/GraphQLUtilsTest.php`

**Interfaces:**
- Consumes: nothing from earlier tasks.
- Produces:
  - `Anorm\GraphQL\GraphQLUtils::methodResolver(Type $type, string $method, string $field): callable`
  - `GraphQLUtils::createField(string $name, Type $type, string $method, $isNullable = false)` and `::createListField(...)`, both returning a `SimPod\GraphQLUtils\Builder\FieldBuilder`
...
Expected: errors, `Class 'Anorm\GraphQL\Mapper' not found` and `Class 'Anorm\GraphQL\GraphQLUtils' not found`.

- [ ] **Step 3: Write the implementation**

`src/GraphQLUtils.php` (moved unchanged from emdc-events, apart from the namespace):
...
```bash
git add src test
git commit -m "feat: runtime GraphQLUtils, Mapper and MangoInput"
```
...
Expected: all suites pass (67 tests), phpcs clean, phpstan `[OK] No errors`.
...
- [ ] **Step 1: Write `README.md`** with these sections, in this order:
  ...
  2. **Install** — `composer require saygoweb/anorm-graphql`. State the supported range: PHP `^7.4 || ^8.0`, `webonyx/graphql-php ^14.8`, `simpod/graphql-utils ^0.5.3`, `php-di/php-di ^6.0`, `saygoweb/anorm ^3.2`.
...
Expected: green on both. PHP 8.3 has only been verified for the no-DB suites so far, so treat a failure in the integration suite there as a real finding, most likely a deprecation made fatal by `failOnWarning`.
